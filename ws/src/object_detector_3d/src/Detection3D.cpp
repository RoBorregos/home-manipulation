...
class Detect3D
{
  ...
  actionlib::SimpleActionServer<object_detector_3d::DetectObjects3DAction> as_;
  actionlib::SimpleActionServer<object_detector_3d::GetPlacePositionAction> place_as_; // TODO: Type of simple as
...
public:
  ...
  void handleActionServer(const object_detector_3d::DetectObjects3DGoalConstPtr &goal)
  {
    ROS_INFO_STREAM("Action Server Detect3D - Goal Received");
    biggest_object_ = goal->force_object.detections.size() == 0;
    force_object_ = !biggest_object_ ? goal->force_object.detections[0] : object_detector_2d::objectDetection();
  ...
  moveit_msgs::CollisionObject generateCollisionObject(const std::string &id, const ObjectParams &object_found)
  {
    moveit_msgs::CollisionObject collision_object;
    collision_object.header.frame_id = BASE_FRAME;
    collision_object.id = id;
    collision_object.meshes.push_back(*object_found.mesh);
    collision_object.mesh_poses.push_back(object_found.center.pose);
    collision_object.operation = collision_object.ADD;
    return collision_object;
  }

  moveit_msgs::CollisionObject generateCollisionObject(const std::string &id, const ObjectParams &object_found, float scaleFactor)
  {
    moveit_msgs::CollisionObject collision_object;
    collision_object.header.frame_id = BASE_FRAME;
    collision_object.id = id;
    shape_msgs::Mesh mesh = *object_found.mesh;
