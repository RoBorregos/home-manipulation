...
from frida_manipulation_interfaces.srv import Gripper, GripperResponse, MoveJointSDK, MoveVeloJointSDK, AimToPointSDK, MoveArm
...
class ArmServer:
    ...
    def __init__(self):
        ...
        # ARM GROUP STATES
        self.ARM_HOME = rospy.get_param(
            "ARM_HOME", [0.0, 0.0, 0.0, -1.5708, 1.5708, 0.7854])
        self.ARM_CALIBRATION = [-1.57, 0.0, -
                                3.1416 / 4, 0, -3.1416 / 4, -2.356]
        self.ARM_NAV = [-1.5708, -1.0472, -1.0472, 1.5708, 0.0, -0.7854]
        self.ARM_PREGRASP = rospy.get_param(
        ...
        self.ARM_RIGHT_SEAT = [-3.1415, -
                               0.5759, -1.5708, 0.0, 0.191986, 0.6981]
        ...
        self.arm_group = moveit_commander.MoveGroupCommander(
            self.ARM_GROUP, wait_for_servers=0)
        self.gripper_group = moveit_commander.MoveGroupCommander(
            self.GRIPPER_GROUP, wait_for_servers=0)
        ...
        self.get_angle = rospy.ServiceProxy(
            '/xarm/get_servo_angle', GetFloat32List)
        ...
        self.gripper_server = rospy.Service(
            "/gripper_service", Gripper, self.handle_gripper)
        self.arm_joint_sdk = rospy.Service(
            '/move_joint_sdk', MoveJointSDK, self.handle_move_joint_sdk)
        self.arm_joint_velocity_sdk = rospy.Service(
            '/move_joint_velocity_sdk', MoveVeloJointSDK, self.handle_move_joint_by_velocity)
        self.aim_to_point_sdk = rospy.Service(
            "/aim_to_point_sdk", AimToPointSDK, self.handle_aim_to_point_sdk)

        self.tfBuffer = tf2_ros.Buffer()
        self.listener = tf2_ros.TransformListener(self.tfBuffer)

        rospy.loginfo(
            f"[INFO] Named targets available: {self.arm_group.get_named_targets()}")

        rospy.wait_for_service('/xarm/move_line')
        rospy.set_param('/xarm/wait_for_finish', True)

        self.moveArmSrv = rospy.Service("/move_arm", MoveArm, self.moveARM)
        rospy.spin()
    ...
    def arm_cb(self, goal):
        ...
        self.arm_group.set_goal_orientation_tolerance(
            np.deg2rad(goal.orientation_tolerance))

        if goal.pose_target != Pose():
            result.success = self.plan_arm(
                goal.pose_target, goal.planning_time, goal.planning_attempts)
        else:
            if goal.target_delta_x != 0 or goal.target_delta_y != 0:
                ...
                if goal.predefined_position in self.arm_group.get_named_targets():
                    get_named_target_dict = self.arm_group.get_named_target_values(
                        goal.predefined_position)
                    get_named_target_values = list(
                        get_named_target_dict.values())
                    print(
                        f"Moving to predefined position: {goal.predefined_position} with values: {get_named_target_values}")
                    result.success = self.move_joints(get_named_target_values)
                elif goal.predefined_position in self.defined_states:
                    result.success = self.move_joints(
                        self.defined_states[goal.predefined_position])
                else:
                    result.success = False
    ...
    def moveARM(self, joints, speed, enable_octomap=True):
        try:
            ...
            if joints == [] or joints is None or len(joints) != 6:
                rospy.logerr("Invalid joint values len: " +
                             str(len(joints)) + " Expected 6")
                return False
    ...
    def move_joints(self, joint_values):
        ...
        distance = math.sqrt((joint_values[0] - current_joints[0])**2 + (joint_values[1] - current_joints[1])**2 + (joint_values[2] - current_joints[2])**2 + (
            joint_values[3] - current_joints[3])**2 + (joint_values[4] - current_joints[4])**2 + (joint_values[5] - current_joints[5])**2)
    ...
    def plan_arm(self, pose, p_time, p_attempts):
        ...
        pose_distance = math.sqrt((pose.position.x - current_pose.position.x)**2 + (
            pose.position.y - current_pose.position.y)**2 + (pose.position.z - current_pose.position.z)**2)
    ...
    def return_to_default_pose_horizontal(self, request):
        ...
        try:
            ...
            req.pose = [-1.5707963705062866, -0.6108652353286743, -1.5707963705062866,
                        3.1415927410125732, -0.6108652353286743, -2.356194496154785]
            joint_move(req)
    ...
    def handle_move_joint_sdk(self, request):
        ...
        try:
            if (self.mode != "Cartesian"):
                ...
                self.set_mode_cartesian()
            req.pose = [actual_pose[0], actual_pose[1], actual_pose[2],
    ...
    def handle_aim_to_point_sdk(self, request):
        ...
        try:
            point_pose_to_xarm = self.tfBuffer.transform(
                request.target_position, "BaseBrazo")
            angle_towards_point = math.atan2(
                point_pose_to_xarm.point.y, point_pose_to_xarm.point.x)
